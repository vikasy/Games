...
// Background: richer navy-to-indigo with more color to contrast against graphics
...
// Outer ring: brighter gradient for better pop
...
// Stronger ring stroke for definition
...
orbGradient.draw(in: orbPath, relativeCenterPosition: NSPoint(x: 0.05, y: 0.25))
...
// Sparkle glints — medium
let glints = [NSPoint(x: orbRect.midX + 150, y: orbRect.midY + 170),
              ...
              NSPoint(x: orbRect.midX + 80, y: orbRect.midY + 30),
              NSPoint(x: orbRect.midX - 90, y: orbRect.midY + 110)]
for (i, pt) in glints.enumerated() {
    let color = i % 2 == 0 ? NSColor(calibratedRed: 255/255, green: 230/255, blue: 130/255, alpha: 1)
...
// Tiny tiny sparkle dots scattered across the orb
let tinyStars = [
...
// Orbit symbols — brighter colors + glow outline for contrast
let symbolFont = NSFont(name: "HelveticaNeue-BoldItalic", size: 110) ?? NSFont.systemFont(ofSize: 110, weight: .bold)
let defaultColor = NSColor(calibratedRed: 100/255, green: 255/255, blue: 240/255, alpha: 1)
let spadeColor = NSColor(calibratedRed: 110/255, green: 240/255, blue: 150/255, alpha: 1)
let hashColor = NSColor(calibratedRed: 255/255, green: 195/255, blue: 90/255, alpha: 1)

func drawLaptopIcon(center: NSPoint) {
    // Glow behind laptop
    let glowRect = NSRect(x: center.x - 90, y: center.y - 70, width: 180, height: 140)
    NSColor(calibratedRed: 120/255, green: 170/255, blue: 255/255, alpha: 0.25).setFill()
    NSBezierPath(ovalIn: glowRect).fill()

    let bodyWidth: CGFloat = 130
    let bodyHeight: CGFloat = 90
    let bodyRect = NSRect(x: center.x - bodyWidth/2, y: center.y - bodyHeight/2, width: bodyWidth, height: bodyHeight)
    let screenRect = bodyRect.insetBy(dx: 8, dy: 14)
    let baseRect = NSRect(x: bodyRect.minX - 10, y: bodyRect.minY - 24, width: bodyWidth + 20, height: 26)

    let screenGradient = NSGradient(colorsAndLocations:
        (NSColor(calibratedRed: 130/255, green: 175/255, blue: 255/255, alpha: 1), 0),
        (NSColor(calibratedRed: 255/255, green: 155/255, blue: 220/255, alpha: 1), 1)
    )!
    NSColor(calibratedWhite: 0.15, alpha: 1).setFill()
    NSBezierPath(roundedRect: bodyRect, xRadius: 12, yRadius: 12).fill()
    screenGradient.draw(in: NSBezierPath(roundedRect: screenRect, xRadius: 8, yRadius: 8), angle: 90)
    NSColor(calibratedWhite: 0.92, alpha: 1).setFill()
    NSBezierPath(roundedRect: baseRect, xRadius: 10, yRadius: 10).fill()
    NSColor(calibratedWhite: 0.7, alpha: 1).setStroke()
    let edge = NSBezierPath(roundedRect: baseRect, xRadius: 10, yRadius: 10)
    edge.lineWidth = 3
    edge.stroke()
}

let orbitSymbols = ["laptop", "#", "🍬", "♠", "pq"]
for (i, sym) in orbitSymbols.enumerated() {
    let angle = CGFloat(i) * (.pi * 2 / CGFloat(orbitSymbols.count)) + .pi/2
    let radius: CGFloat = 250
    let center = NSPoint(x: orbRect.midX + cos(angle) * radius,
                         y: orbRect.midY + sin(angle) * radius)

    if sym == "laptop" {
        drawLaptopIcon(center: NSPoint(x: center.x, y: center.y + 50))
        continue
    }

    // Push # and pq up and toward center so they stay in the magenta orb area
    var drawCenter = center
    if sym == "#" {
        drawCenter.y += 130
        drawCenter.x += 70  // toward center
    } else if sym == "pq" {
        drawCenter.y += 130
        drawCenter.x -= 70  // toward center
    }

    // Glow behind each symbol for readability on dark bg
    let glowRect = NSRect(x: drawCenter.x - 55, y: drawCenter.y - 55, width: 110, height: 110)
    NSColor(calibratedRed: 80/255, green: 120/255, blue: 200/255, alpha: 0.2).setFill()
    NSBezierPath(ovalIn: glowRect).fill()

    let color: NSColor
    switch sym {
    case "♠": color = spadeColor
    case "#": color = hashColor
    default: color = defaultColor
    }

    // Draw shadow behind symbol for legibility
    let shadowAttrs: [NSAttributedString.Key: Any] = [
        .font: symbolFont,
        .foregroundColor: NSColor(calibratedWhite: 0, alpha: 0.5)
    ]
    let s = sym as NSString
    let sz = s.size(withAttributes: shadowAttrs)
    s.draw(at: NSPoint(x: drawCenter.x - sz.width/2 + 3, y: drawCenter.y - sz.height/2 - 3), withAttributes: shadowAttrs)

    let attrs: [NSAttributedString.Key: Any] = [.font: symbolFont, .foregroundColor: color]
    s.draw(at: NSPoint(x: drawCenter.x - sz.width/2, y: drawCenter.y - sz.height/2), withAttributes: attrs)
}

// f(A) text with stronger glow
let fShadow = NSShadow()
fShadow.shadowBlurRadius = 35
fShadow.shadowColor = NSColor(calibratedWhite: 0, alpha: 0.9)
fShadow.shadowOffset = NSSize(width: 0, height: -8)
...
// Bright glow behind text
g.saveGState()
let textGlowCenter = CGPoint(x: orbRect.midX, y: orbRect.midY + 40)
let textGlowColors = [
...
g.drawRadialGradient(textGlow, startCenter: textGlowCenter, startRadius: 0, endCenter: textGlowCenter, endRadius: 180, options: [])
g.restoreGState()

formula.draw(at: NSPoint(x: orbRect.midX - formulaSize.width/2, y: orbRect.midY - formulaSize.height/2 + 40),
             withAttributes: formulaAttrs)

// Glow behind controller for separation from dark bg
g.saveGState()
...
let outputPath = "/Users/vikasyadav/Library/CloudStorage/OneDrive-Personal/Documents/GitHub/Games/fa_arcade_icon.png"
